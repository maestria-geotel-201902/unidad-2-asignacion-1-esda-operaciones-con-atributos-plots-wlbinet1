...
```{r}
library(magrittr)
en17$Código <- en17$Código %>% mutate(ENLACE = ifelse(nchar(Código)==3, paste0('0', Código), Código))
```
...
```{r}
en17$ENLACE
match(en17$E_ENLACE , prov$ENLACE)
```
...
```{r}
... %>%
  dplyr::select(contains('Desde noviembre del año pasado hasta la fecha, ¿Alguna de las personas de este hogar tuvo un accidente de tránsito mientra conducía un vehículo, motor o bicicleta?: Si')) %>%
  plot(breaks = 'jenks')
```
...
#Rótulos: nombres de las provincias
proven17 %>%
  dplyr::select(contains('Desde noviembre del año pasado hasta la fecha, ¿Alguna de las personas de este hogar tuvo un accidente de tránsito mientra conducía un vehículo, motor o bicicleta?: Si'), TOPONIMIA) %>%
  gather(variable, valor, -geom, -TOPONIMIA) %>% 
  ggplot() + aes(fill = valor) + geom_sf(lwd = 0.2) +
  facet_wrap(~variable) + theme(text = element_text(size = 10)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds'), trans = 'log10') +
...
```{r}
proven17 %>% st_drop_geometry() %>%
  dplyr::select(TOPONIMIA, contains('Desde noviembre del año pasado hasta la fecha, ¿Alguna de las personas de este hogar tuvo un accidente de tránsito mientra conducía un vehículo, motor o bicicleta?: Si'))
```
...
proven17 %>%
  ...
  group_by(TOPONIMIA) %>% 
  mutate(pct=round(valor/sum(valor)*100,2)) %>% dplyr::select(-valor) %>% 
  filter(variable %in% 'Desde noviembre del año pasado hasta la fecha, ¿Alguna de las personas de este hogar tuvo un accidente de tránsito mientra conducía un vehículo, motor o bicicleta?: No') %>% 
  ggplot() + aes(fill = pct) + geom_sf(lwd = 0.2) +
  facet_wrap(~variable) + theme(text = element_text(size = 10)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds')) +
  geom_sf_text(aes(label=pct), check_overlap = T, size = 3)
...
```{r}
proven17 %>% st_drop_geometry() %>% filter(ENLACE=='0930') %>% 
  dplyr::select(contains('Desde noviembre del año pasado hasta la fecha, ¿Alguna de las personas de este hogar tuvo un accidente de tránsito mientra conducía un vehículo, motor o bicicleta?: Si'))
```
